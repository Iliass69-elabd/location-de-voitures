<?php 
  ...
  $c = new Control;
  include '../CONTROL/google.php';
  
if (!isset($_SESSION['user_token']) && !isset($_SESSION["login_simple"])) {
        if (isset($_GET['code'])) {
            $token = $client->fetchAccessTokenWithAuthCode($_GET['code']);
            $client->setAccessToken($token['access_token']);

            // get profile info 
            /** @var Google_Service_Oauth2 $oauth2 */
            $google_oauth = new Google_Service_Oauth2($client);
            $google_account_info = $google_oauth->userinfo->get();
            $userinfo = [
                'email' => $google_account_info['email'],
                'prenom_client' => $google_account_info['givenName'],
                'nom_client' => $google_account_info['familyName'],
                'token' => $google_account_info['id'],
            ];

            $r = $c->google_un($userinfo['email'], $userinfo['nom_client'], $userinfo['prenom_client'], $userinfo['token']);

            // save user data into session

            $u = $c->login_google($userinfo['token']);
            if($u) {
            ...
            if((isset($_SESSION["com"]) && isset($_SESSION["rev"]))){   
              header('Location:consulter.php?matricul='.$_SESSION['com']); 
            }
        } else {
...
<body>
    ...
    <div class="nav_container">
      <nav class="nav">
        ...
        <div class="nav-links">
          <a  class="cool-link" href="./profile.php?id=">Profile</a>
...
<script>
     function search(){
            ...
            // console.log(document.getElementById("big_container_car"));
      }
      function min_max(){
      ...
      //     $("#logo_wlc").css("filter" , " brightness(00)"); 
      //     // document.getElementById("logo_wlc").style.filter="brightness(0)"
      //   }
      //   else{
