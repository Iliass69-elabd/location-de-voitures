...
$(document).on("click", "#conf", function (e) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      // $("#conf").css("background-color", "grey");
      // $("#conf").css("color", "white");
      // $("#conf").css("border-color", "transparent");
      // showInfos(data.message);
...
//   var db = $("#fin").val();
//   var fn = $("#debut").val();
//   var timeDiff = Math.abs(fn- db);
//   var differenceDays = Math.floor(timeDiff / (1000 * 60 * 60 * 24));
//   $("#duree").val(differenceDays);
//   console.log(differenceDays);
// // }
// $("#fin").change(set_duree);
// $("#debut").change(set_duree);
    ...
    infos.map((elt) => {
      ...
      document.querySelector("#tbody").innerHTML += `
          <tr id="${elt.id_reserve}">
              ...
              <td> <input type="button" id="update_r" value="Update" class="update btn btn-primary" id="updt"  onclick="updateInfos('${
                elt.id_reserve
        ...
        elt.prix_unitaire
      }')"/> <input type="button" value="Delete" id="delete_r" class="delete btn btn-danger" name="delete" onclick="deleteInfos(${
        elt.id_reserve
      })"/> <a href="contrat.php?id_res=${
        elt.id_reserve
      }" class="btn btn-success contrat_link">Contrat</a>
              ${
                elt.confirmer == "1"
                  ? `<button disabled class="btn btn-secondary"> Confirmer </button>`
                  : `<button value="${elt.id_reserve}" id="conf" class="btn btn-warning"> Confirmer </button>`
              }
              </td>
          </tr>
        `;
...
function searchTable() {
  ...
  for (var i = 0; i < rows.length; i++) {
    var cells = rows[i].getElementsByTagName("td");
    var date = $(cells[4]).html() || $(cells[4]).text();
